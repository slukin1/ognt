...
@supports (color: color-mix(in oklab, white, black)) {
    .rt-Link:where(.rt-underline-auto):where(.rt-high-contrast),
    :where(.rt-CalloutRoot: not(.rt-high-contrast)) .rt-Link:where(.rt-underline-auto), :where(.rt-Text, .rt-Heading):where([data-accent-color]:not(.rt-high-contrast)) .rt-Link:where(.rt-underline-auto) {
...
.rt-SwitchRoot:where(.rt-variant-soft) :where(.rt-SwitchButton)::before {
    background-image: linear-gradient(to right, var(--accent-a4) 40%, transparent 60%), linear-gradient(to right, var(--accent-a4) 40%, transparent 60%), /* Blend with white to increase contrast in dark mode */
    linear-gradient(to right, var(--accent-a4) 40%, var(--white-a1) 60%), linear-gradient(to right, var(--gray-a2) 40%, var(--gray-a3) 60%);
}
