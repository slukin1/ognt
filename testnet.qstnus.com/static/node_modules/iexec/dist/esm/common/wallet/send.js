...
export const sweep = async (
    ...
    try {
        ...
        if (gasFees.gasPrice === undefined) {
            ...
            if (
                (networkGasFees.gasPrice || 0 n) < (networkGasFees.maxFeePerGas || 0 n)
            ) {
                gasFees.gasPrice = undefined;
                gasFees.maxFeePerGas = networkGasFees.maxFeePerGas || 0 n;
                gasFees.maxPriorityFeePerGas = gasFees.maxFeePerGas; // pay all
            } else {
                gasFees.gasPrice = networkGasFees.gasPrice || 0 n;
            }
        }
        if ((gasFees.gasPrice || 0 n) < (gasFees.maxFeePerGas || 0 n)) {
            maxGasPrice = gasFees.maxFeePerGas;
        } else {
            maxGasPrice = gasFees.gasPrice;
        }

        const txFee = maxGasPrice * 21000 n; // send native tx
